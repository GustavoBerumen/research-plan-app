...
Syntax:
  "# Field Label" matches a field by its label in research-plan-template.md
  (matched the same way field keys are derived — case/spacing-insensitive).

  Each bullet under a heading is one criterion:
...
- Essential Knowledge: Introduces and defines key terms early on so that related sections—such as the Problem Statement, Goal, and Objectives—can reference them seamlessly without confusion.
...
- Specificity: Includes a quantifiable metric (%, $, count, rate)
- Feasibility: Scope is narrow enough to be addressed by one research effort
- Significance: States or implies the business/user impact of solving it
