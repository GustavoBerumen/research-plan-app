...
// Run from the repository: node test/manual/rpa-53-preview.cjs
...
http.createServer(async (req, res) => {
  ...
  if (asset === 'index.html') {
    ...
    content = content.replace('<body>', '<body><div class="toolbar" style="padding:12px;background:#fff2cc;color:#111">RPA-53 — MOCK FEEDBACK — dedicated 05ea worktree preview. No AI requests or saved feedback. Add [fail] to a field to simulate a request failure.</div>');
  }
  res.writeHead(200, { 'Content-Type': (types[path.extname(asset)] || 'text/plain') + '; charset=utf-8', 'Cache-Control': 'no-store' });
  res.end(content);
}).listen(8936, '127.0.0.1', () => console.log('MOCK feedback preview: http://127.0.0.1:8936/?test'));
