...
test('Plan details is on the review step too, answers and all, and Change there comes back as well', async (t) => {
  ...
  reveal(details);
  assert.deepEqual(rowsOf(details).map((r) => r[0]), ['Research title', 'Jira Project', 'Lead researcher', 'Other researchers', 'Researcher names', 'Project requester', 'Project decision', 'Research readout']);
