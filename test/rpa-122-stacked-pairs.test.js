...
test('the dates page still asks both dates together, decision above readout in the order a page is read', async (t) => {
  ...
  for (let k = 0; k < 5; k++) plan.querySelector('.step-continue').click();   // six pages since the email address (RPA-99)
  assert.equal(text(plan.querySelector('.step-page-caption')), 'Question 6 of 6');
